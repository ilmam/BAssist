...
- **The Rule of Integrity:** Prefer a continuous, unbroken chain from a Business Need through Business Objectives all the way down to a testable Scenario and the process step that elaborates it. A break in that chain usually means one of two things: missing scope (you forgot to build something) or unnecessary waste (you built something nobody asked for). Process steps without a Stakeholder Need, or without an elaborating FR/Feature, are highlighted as gaps.
