...
In BAssist, each project has **one** Strategic Baseline document with three narrative fields—**Current state**, **Future state**, and **Change strategy**—plus a lifecycle **Status** (`Draft` → `In review` → `Approved`). Related strategy artifacts live beside it on the Strategy hub and Need Spine: Business Objectives, Business Needs, Scope Items (in/out), Assumptions, Constraints, and Risks.
...
2. **Complete the strategy package around the document—not only inside it:**
   - **Strategic alignment:** Link **Business Objectives** ↔ **Business Needs** so every need traces to an enterprise outcome. The baseline narratives should agree with those records; there is no separate “alignment statement” field.
...
- **Upstream:** Executive intent and market reality, expressed as Business Objectives and Business Needs. The baseline synthesizes those into a single approved strategy story.
