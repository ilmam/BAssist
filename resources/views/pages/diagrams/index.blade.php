...
@section('main')
    ...
    <div class="space-y-5">
        <x-card title="{{ __('ui.diagrams') }}">
            <x-slot:titleAside>
                <x-help-trigger topic="diagrams" />
            </x-slot:titleAside>
            <p class="text-sm text-muted-foreground mb-5">{{ __('ui.diagrams_help') }}</p>

            <x-list-filter-panel
                :active-count="! empty($filters['project_id']) ? 1 : 0"
                :clear-url="! empty($filters['project_id']) ? route('diagrams.index', array_filter(['workspace_id' => $filters['workspace_id'] ?? null, 'clear_project' => 1])) : null"
            >
                <form method="GET" action="{{ route('diagrams.index') }}" class="list-filter-panel__form" data-list-filter-form>
                ...
                <div class="kt-scrollable-x-auto">
                    <table class="kt-table table-fixed">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-secondary-foreground">{{ __('ui.diagrams_empty') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        @empty
            <x-card title="{{ __('ui.diagrams') }}">
                <p class="text-sm text-muted-foreground">{{ __('ui.diagrams_no_access') }}</p>
            </x-card>
        @endforelse
    </div>
@endsection
