    ...
    <div class="space-y-5">
        <x-card title="{{ __('ui.solution_requirements') }}">
            ...
            <x-list-filter-panel
                ...
                :clear-url="! empty($filters['project_id']) ? route('solution_requirements.index', array_filter(['workspace_id' => $filters['workspace_id'] ?? null, 'clear_project' => 1])) : null"
            >
                <form method="GET" action="{{ route('solution_requirements.index') }}" class="list-filter-panel__form" data-list-filter-form>
