...
**Hybrid** entities use **materialized** form blades: the same outer shell, but with one `Form::field(...)` line per `FormFieldAttribute` on the edit DTO. Generated by `entity:eject`, `make:entity --profile=hybrid`, or `entity:materialize-form`. See [entity-scaffolding.md](entity-scaffolding.md#materialized-forms).
